...
class DetailFotoScreen extends StatefulWidget {
  ...
  @override
  State<DetailFotoScreen> createState() => _DetailFotoScreenState();
}

class _DetailFotoScreenState extends State<DetailFotoScreen> with SingleTickerProviderStateMixin {
  ...
  void _loadSavedStatus() async {
    ...
    if (userId != null) {
      try {
        print('Fetching saved status for user: $userId'); // Debug log
        ...
        if (response.statusCode == 200) {
          ...
          if (data['success'] == true) {
            setState(() {
              ...
              final savedPhotoIds = {
                for (var photo in data['saved_photos'])
                  int.parse(photo['FotoID'].toString())
              };

              for (var photoInfo in widget.photoInfos) {
                photoInfo.isSaved = savedPhotoIds.contains(photoInfo.id);
                savedStatus[photoInfo.id] = photoInfo.isSaved;
              }
            });
          }
        }
    ...
    // Check saved status from SharedPreferences
    for (var photoInfo in widget.photoInfos) {
      bool? isSaved = prefs.getBool('saved_${photoInfo.id}');
      if (isSaved != null) {
        setState(() {
          photoInfo.isSaved = isSaved;
        });
      }
    }
  }
...
// Ubah fungsi _buildReplyItemWithCreatorLabel untuk memperbaiki perbandingan ID
  ...
  Widget _buildCommentItem(CommentModel comment, int commentIndex) {
    // Ambil jumlah balasan dari _repliesMap
    final replyCount = _repliesMap.containsKey(comment.id) ?
    (_repliesMap[comment.id]?.length ?? 0) : 0;

    // Variabel lokal untuk melacak loading state balasan
    final bool isLoadingRepliesForThisComment =
        _isLoadingCommentReplies && _currentLoadingCommentId == comment.id;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
                        ...
                        child: Material(
                          ...
                          child: InkWell(
                            onTap: () {
                              ...
                              _toggleRepliesImproved(comment.id);
                            },
                            child: Padding(
          ...
          // Tampilkan loading indicator atau balasan
          if (replyCount > 0 && (comment.showReplies || isLoadingRepliesForThisComment))
            AnimatedSwitcher(
              duration: const Duration(milliseconds: 300),
              child: isLoadingRepliesForThisComment
                  ? Padding(
                padding: const EdgeInsets.only(left: 48, top: 12),
                key: ValueKey('loading-${comment.id}'),
                child: Center(
                  child: SizedBox(
                    width: 20,
                    height: 20,
                  ...
                  : Column(
                key: ValueKey('replies-${comment.id}'),
                children: [
                  ..._repliesMap[comment.id]?.map((reply) {
                    return Padding(
                      padding: const EdgeInsets.only(left: 48, top: 12),
  ...
  Widget _buildReplyItem(ReplyModel reply) {
    ...
    final bool isPhotoOwner = widget.photoInfos[currentIndex].uploaderId.isNotEmpty &&
        reply.userId.toString() == widget.photoInfos[currentIndex].uploaderId;
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => DetailFotoScreen(
              ...
              onLoadMore: (int page, {String? tag}) async {
                // Implement load more logic here
                // You can call a method from your parent widget or implement directly
                if (widget.onLoadMore != null) {
                  return widget.onLoadMore!(page, tag: tag);
                }
              },
            ),
          ),
